...
if local_rank == 0:
    ...
    model = ViTSTCNModel(para, logger=logger, 
                    save_path=path.join('/apdcephfs/share_1290939/qiangqwu/STCN_ft', long_id, long_id) if long_id is not None else None, 
...
# we do not need this, instead we directly load the mae weight
...
# else:
#     total_iter = 0
# total_iter = model.load_model('/apdcephfs/private_qiangqwu/Projects/STCN/saves/Jul29_13.57.25_retrain_s03/Jul29_13.57.25_retrain_s03_checkpoint.pth')
# print('Previously trained model loaded!')
...
# # load the s0 model here
# network_path = '/apdcephfs/share_1290939/qiangqwu/STCN_ft/Jan27_04.35.34_retrain_s0/Jan27_04.35.34_retrain_s0_300000.pth'
# print('load %s' %network_path)
# model.load_network(network_path)
# print('Previously trained network loaded!')
...
def renew_vos_loader(max_skip):
    ...
    # the settings for DAVIS evaluation
    train_dataset = ConcatDataset([davis_dataset]*5 + [yv_dataset])

    # train_dataset = ConcatDataset([yv_dataset])
...
if para['stage'] == 0:
    ...
    train_sampler, train_loader = renew_vos_loader(max_interval)  # start with 5
    # train_sampler, train_loader = renew_vos_loader(5)  # start with 5
...
try:
    ...
    if not para['debug'] and model.logger is not None and total_iter>50000:
