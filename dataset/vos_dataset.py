    ...
    def __getitem__(self, idx):
        ...
        while trials < 5:   
            ...
            if len(labels) == 0:
                ...
                target_object = np.random.choice(labels)
                # we do not need the 2nd object for saving computational cost
                has_second_object = False #(len(labels) > 1)
                if has_second_object:   # we randomly select another target sa the second object. 
                    labels = labels[labels!=target_object]
                    second_object = np.random.choice(labels)
                break
        ...
        cls_gt = np.zeros((3, self.img_size, self.img_size), dtype=np.int) # we only use three frames
        cls_gt[tar_masks[:,0] > 0.5] = 1
        # cls_gt[sec_masks[:,0] > 0.5] = 2 # never appears
