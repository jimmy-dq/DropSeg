...
class BootstrappedCE(nn.Module):
    def __init__(self, start_warm=20000, end_warm=70000, top_p=0.3):
    # def __init__(self, start_warm=40000, end_warm=70000, top_p=0.15):
        super().__init__()
...
class LossComputer:
    ...
    def compute(self, data, it):
        ...
        for i in range(1, s):
            ...
            # data['cls_gt']: 4, 3, 384, 384; 3: three frames
            for j in range(b):
                if selector is not None and selector[j][1] > 0.5: # we have the 2nd target
                    loss, p = self.bce(data['logits_%d'%i][j:j+1], data['cls_gt'][j:j+1,i], it) # loss on the 1 (0,1,2) frame
                else:
