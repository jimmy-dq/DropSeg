...
class HyperParameters():
    def parse(self, unknown_arg_ok=False):
        ...
        # Data parameters
        parser.add_argument('--static_root', help='Static training data root', default='/dockerdata/static')
        parser.add_argument('--bl_root', help='Blender training data root', default='../BL30K')
        parser.add_argument('--yv_root', help='YouTubeVOS data root', default='/home/user/HDD_Data/YouTube19')
        parser.add_argument('--davis_root', help='DAVIS data root', default='/home/user/Data/DAVIS')
        ...
        parser.add_argument('--num_workers', help='Number of datalaoder workers per process', type=int, default=8)   #8
        parser.add_argument('--img_size', help='image size', type=int, default=384)
        ...
        parser.add_argument('--warm_up_iter', help='warm_up_iter', type=int, default=-1)

        parser.add_argument('--lr', help='Initial learning rate', type=float)
        parser.add_argument('--gamma', help='LR := LR*gamma at every decay step', default=0.1, type=float)
        parser.add_argument('--weight_decay', help='weight_decay', default=1e-7, type=float) #1e-7
        parser.add_argument('--droppath_rate', help='droppath_rate', default=0.1, type=float) #0.25

        # Loading
        parser.add_argument('--load_network', help='STCN: Path to pretrained network weight only')
        parser.add_argument('--load_model', help='STCN: Path to the model file, including network, optimizer and such')
        parser.add_argument('--pretrained_net_path', help='pretrained_net_path', default='')
        ...
        if self.args['stage'] == 0:
            ...
            # for main training on YoutubeVOS + DAVIS
            self.args['lr'] = none_or_default(self.args['lr'], 2e-5) #2e-5
            self.args['batch_size'] = none_or_default(self.args['batch_size'], 4)
            self.args['iterations'] = none_or_default(self.args['iterations'], 210000) 
